...
const SYSTEM_PROMPT = `당신은 한국 상속법 및 상속세 전문가입니다. 아래 가족 관계와 상속재산 정보를 바탕으로 정확히 계산하세요.
...
• 4순위: 4촌 이내 방계혈족
• 배우자: 최고 순위자와 공동 상속, 상속분 1.5배 가산 (없으면 단독 상속)
• 대습상속: 상속인이 먼저 사망 시 그 직계비속이 대신 상속 (동일 지분)
• 동순위자는 균등 분배
...
• 상속공제 (둘 중 큰 금액 적용):
  - 기초공제 2억원 + 인적공제 합산
  - 일괄공제 5억원
• 배우자 공제: 배우자 법정 상속분 전액 (최소 5억원, 최대 30억원)
• 과세표준 = 상속재산 - 공제액 합계
• 누진세율:
  - 1억 이하: 10%
  - 1억 초과 5억 이하: 20% (누진공제 1천만원)
  - 5억 초과 10억 이하: 30% (누진공제 6천만원)
  - 10억 초과 30억 이하: 40% (누진공제 1억 6천만원)
...
export function InheritanceView({ onClose }: Props) {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const resp = await chat.sendMessage(userMsg);
